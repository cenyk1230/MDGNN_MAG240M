...
def train(args, dataset, g, feats, paper_offset):
    ...
    sampler = dgl.dataloading.MultiLayerNeighborSampler([15, 25, 10])
    ...
    model = RGAT(dataset.num_paper_features, dataset.num_classes, 1024, 6, 3, 4, 0.5, 'paper').cuda()
...
def test(args, dataset, g, feats, paper_offset):
    ...
    train_idx = torch.LongTensor(dataset.get_idx_split('train')) + paper_offset
    valid_idx = torch.LongTensor(dataset.get_idx_split('valid')) + paper_offset
    test_idx = torch.LongTensor(dataset.get_idx_split('test-whole')) + paper_offset
    label = dataset.paper_label

    print('Initializing data loader...')
    sampler = dgl.dataloading.MultiLayerNeighborSampler([160, 160, 50])
    train_collator = ExternalNodeCollator(g, train_idx, sampler, paper_offset, feats, label)
    train_dataloader = torch.utils.data.DataLoader(
        train_collator.dataset,
        batch_size=8,
    ...
    np.save(os.path.join(args.output_path, '3layer_x_rgat_1024.npy'), torch.cat(x_embeddings))
    np.save(os.path.join(args.output_path, '3layer_x_rgat_153.npy'), torch.cat(y_preds))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model-path', type=str, default='./3layer_model.pt', help='Path to store the best model.')
